...
import Sid from './SideBar5.module.css'
...
export default function Sidebar() {
  ...
  return (
    <aside className={Sid["sidebar"]}>
      <nav>
        <ul>
          {menuItems.map((item, index) => (
            <li key={index} className={item.active ? Sid['active'] : ''}>
              <Link to={item.path} className={Sid["sidebar-link"]}>
                <span className={Sid["icon"]}>{item.icon}</span>
                <span className={Sid["menu-text"]}>{item.text}</span>
              </Link>
            </li>
          ))}
        </ul>
